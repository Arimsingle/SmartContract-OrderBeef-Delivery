...
contract OrderBeef{
    ...
    function setOrder(address _from, string memory _menu, string memory _level, uint256 _price, uint256 _amount, string memory _moreDetails, string memory _time) public returns (bool _sucess){
        require(balance[_from]>=_price,"your balance not enough !");
        balance[_from] = balance[_from].sub(_price);
        Beef memory _beefObj = Beef(_menu, _level, _price);
    ...
    function getOder(address _from)public view returns(Order[] memory){
        return order[_from];
    }
